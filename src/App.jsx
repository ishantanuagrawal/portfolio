...
const App = () => {
  ...
  // State to hold data fetched from our build-time generated JSON
  const [REPO, setRepo] = useState({ photography: {}, videography: {}, reels: [], intro: '' });
  const [isLoading, setIsLoading] = useState(true);
  const [errorMsg, setErrorMsg] = useState(null);

  // --- FETCH ASSETS.JSON (Generated by GitLab Pipeline) ---
  useEffect(() => {
    const loadAssets = async () => {
      try {
        // Fix for GitHub Pages: Prepend Vite's base URL to the fetch request
        const response = await fetch(`${import.meta.env.BASE_URL}assets.json`);
        if (!response.ok) {
          throw new Error('Failed to load assets.json. Did the pipeline run the script?');
        }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-zinc-400 text-xs tracking-widest uppercase">Loading Portfolio...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 font-sans selection:bg-zinc-200 overflow-x-hidden">
      ...
      <main id="portfolio" className="max-w-7xl mx-auto px-6 pb-24 border-t border-zinc-100 pt-10">
        ...
        {errorMsg && (
           <div className="bg-red-50 text-red-900 p-6 rounded-md border border-red-200 mb-8">
             <p className="font-bold">Error Loading Google Drive Assets</p>
             <p>{errorMsg}</p>
             <p className="mt-2 text-sm">Please ensure the GitLab pipeline has run and generated `public/assets.json` using the Google Drive script.</p>
           </div>
        )}
              ...
              {displayedVideos.map((video) => (
                ...
                >
                  <div className="relative aspect-video overflow-hidden rounded-sm mb-4 bg-zinc-100 shadow-sm">
                    {/* Note: Google drive limits video streaming, we use thumbnails where possible */}
                    <img src={video.url} className="w-full h-full object-cover transition-transform group-hover:scale-105" alt="" loading="lazy" />
        ...
        {activeTab === 'reels' && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in duration-500">
            {REPO.reels.map((reel) => (
              ...
              >
                <img src={reel.url} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="" loading="lazy" />
                <div className="absolute inset-0 flex items-center justify-center bg-black/10 group-hover:bg-black/30 transition-colors">
      ...
      <section id="about" className="py-24 px-6 bg-zinc-50 border-y border-zinc-100">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative aspect-[4/5] max-w-sm mx-auto">
             ...
             <img src={REPO.intro || 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&q=80&w=800'} className="w-full h-full object-cover relative z-10 shadow-2xl transition-all duration-700" alt="Shantanu" />
          </div>
        </div>
      </section>
